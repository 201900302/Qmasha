  ...
  <div class="boutiqueSection py-3" >
    ...
    <div class="container" >
        <div class="boutiqueInfo">
          ...
          <div style="margin-top: -50px;"><img style="height: 200px; width: 200px; box-shadow: 10px 10px 5px #ccc;" src="{{url('uploud/vendor_images/'.$boutique->photo)}}" class="rounded-circle" alt="" ></div>
          <div class="boutiqueTextInfo pl-4">
            <div style="display: flex;">
                <i class="bi bi-shop"></i><h5 style="margin-top: 7px; padding-left:10px;"><a href="{{route('boutique.details', $boutique->id)}}">{{$boutique->boutiqueName}}</a></h5>
                </div>
                <div style="display: flex;">
                    <i class="bi bi-calendar4-week"></i><p style="margin-top: 7px; padding-left:10px;">Since {{$boutique->vendor_join_date}}</p>
                    </div>
              <p>{{$boutique->vendor_description}}</p>
              
              
              <div style="display: flex;">
                <i class="bi bi-instagram"></i><p style="margin-top: 5px; padding-left:10px;"><a href="{{$boutique->account_URL}}">Click for {{$boutique->boutiqueName}}`s Instagram Page</a></p>
                </div>
          </div>
      ...
      <div class="row mb-5">
        <div class="col-md-9 order-2">
          ...
          <div class="row">
            <div class="col-md-12 mb-5">
              <div class="float-md-left mb-4"><h2 class="h5 text-black"> All {{$boutique->boutiqueName}} Products <span class="badge rounded-pill bg-secondary text-light"> {{ count($boutique_products) }}</span></h2><hr></div>
        ...
        <div class="row mb-5">
          ...
          <div class="col-md-3 order-1 mb-5 mb-md-0">
            ...
            @if (count($boutique_products) > 1)
          <div class="border p-4 rounded mb-4">
  
            <form action="{{route('shop.boutique.filter')}}" method="POST">
              @csrf
              <input type="hidden" name="vendor_id" value="{{$boutique->id}}">
              ...
              <div class="mb-4">
                ...
                @php
                $products = App\Models\Product::where('category_id', $category->id)->where('vendor_id', $boutique->id)->get();
                @endphp
